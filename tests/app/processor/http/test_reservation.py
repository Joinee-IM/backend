...
class TestRejectInvitation(AsyncTestCase):
    def setUp(self) -> None:
        ...
        self.wrong_reservation_members_manager = [
            do.ReservationMember(
                reservation_id=self.reservation_id,
                account_id=self.account_id,
                is_manager=True,
        ...
        self.wrong_reservation_members_joined = [
            do.ReservationMember(
                reservation_id=self.reservation_id,
                account_id=self.account_id,
                is_manager=False,
    ...
    @patch('app.persistence.database.reservation_member.browse', new_callable=AsyncMock)
    @patch('app.persistence.database.reservation.get_manager_id', new_callable=AsyncMock)
    async def test_happy_path(
        self, mock_get_manager: AsyncMock, mock_browse: AsyncMock, mock_reject: AsyncMock,
        mock_context: MockContext,
    ):
        mock_context._context = self.context
        mock_get_manager.return_value = self.manager_id
        mock_browse.return_value = self.reservation_members
    ...
    async def test_no_permission(
        ...
        mock_context._context = self.context
        mock_get_manager.return_value = self.wrong_manager_id
        mock_browse.return_value = self.wrong_reservation_members_manager
    ...
    async def test_no_permission(
        ...
        mock_context._context = self.context
        mock_get_manager.return_value = self.manager_id
        mock_browse.return_value = self.wrong_reservation_members_joined
