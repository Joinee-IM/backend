...
async def auth(request: Request):
    ...
    if search_str.encode('utf-8') in request['query_string']:
        ...
        response = RedirectResponse(url=f"{service_config.url}/", status_code=302)
        response.set_cookie(key="account_id", value=str(account_id), httponly=True, samesite='none', secure=True, domain='http://localhost:3000')
        response.set_cookie(key="token", value=str(token), httponly=True, samesite='none', secure=True, domain='http://localhost:3000')
        return response
