...
async def login(data: LoginInput, response: FastAPIResponse) -> Response[LoginOutput]:
    ...
    response.set_cookie(key="account_id", value=str(account_id), httponly=True, samesite='none')
    response.set_cookie(key="token", value=str(token), httponly=True, samesite='none')
